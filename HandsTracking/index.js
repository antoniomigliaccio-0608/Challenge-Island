...
// 20 = pinky tip
// 0 = wrist
let TRACKED_KEYPOINT_INDEX = 10;    // Which hand point to use for interaction
let TRACKED_HAND = 'First';          // Which hand to track: 'Left', 'Right', or 'First' (first detected)
...
function setup() {
  ...
  cam.onReady(() => {
    ...
    let options = {
      maxHands: 10,          // Detect up to 2 hands
      runtime: 'mediapipe', // Use MediaPipe for better performance
      flipHorizontal: true // Don't flip in ML5 - cam.mapKeypoint() handles mirroring
    };
...
// ==============================================
function draw() {
  background(255);  // Dark gray background
  ...
  if(cursor && cursor.x >= posx+10){ // cursor puo essere insesitente e se cio posto in un if crasha tutto
      if(posx <= cursor.x && cursor){
        posx = posx + 10
      }
  ...
  }else if( cursor && cursor.x <= posx){
    if(cursor && posx >= cursor.x){
       posx = posx - 10
    }
  }


  if(cursor && cursor.y <= posy){
    posy = posy - 10
  }
...
function drawTrackedCursor() {
  ...
  // Draw cursor at tracked position with depth-based size
  push();
  fill(CURSOR_COLOR[0], CURSOR_COLOR[1], CURSOR_COLOR[2]);
  noStroke();
  ellipse(cursor.x, cursor.y, depthSize, depthSize);
